...
(: list-sort (All (A) (-> (-> A A Boolean) (Listof A) (Listof A))))
(define (list-sort f l)
  (sort l f))

(: remp (All (A) (-> (-> A Any) (Listof A) (Listof A))))
(define (remp f l)
  (filter-not f l))

(: call-with-string-output-port (All (B) (-> (-> Output-Port B) String)))
(define (call-with-string-output-port f)
  (define p (open-output-string))
  (f p)
  (get-output-string p))

(: exists (All (A B) (-> (-> A B) (Listof A) (U #f B))))
(define (exists f l)
  (ormap f l))

(: for-all (All (A B C) (-> (-> A B : C) (Listof A) B)))
(define (for-all f l)
  (andmap f l))

(: find (All (A) (-> (-> A Any) (Listof A) (U #f A))))
(define (find f l)
  (cond [(memf f l) => car] [else #f]))

(define memp memf)

(: var*? (-> (Listof Any) Boolean))
...
(define-type Constraint-Store
  (List (Listof Any)
        ...
        (Listof Any)
        (Listof Any)))
(define-type CS Constraint-Store)

(define-type ES
  (Pairof (Listof Eigen) (Listof Var)))

(define-type Eigen
  (Vectorof Any))

(define-type Eigen-Tag (Vector 'eigen-tag))

(define-type Var
  (Vectorof Any))

(define-type Var-Tag (Vector 'var-tag))

(define-type Sty
  (Listof (Pairof Any Any)))

;; ---

(define empty-c '(() () () () () () ()))

(define eigen-tag : Eigen-Tag (vector 'eigen-tag))

(define var-tag : Var-Tag (vector 'var-tag))
...
(: rhs (All (A B) (-> (Pairof A B) B)))
(define (rhs pr)
  (cdr pr))

(: lhs (All (A B) (-> (Pairof A B) A)))
(define (lhs pr)
  (car pr))
...
(define (walk u S)
    (cond
      ((and (var? u) (assq u S)) =>
       (lambda ((pr : (Pairof Any Any))) (walk (rhs pr) S)))
      (else u)))

(: prefix-S (-> Sty Sty Sty))
(define (prefix-S S+ S)
    (cond
      ((eq? S+ S) '())
      (else (cons (car S+)
