...
bg:
- replace `timer` with `Timer` (main returns Void)
- remove:
  - optional args to `advance`, never supplied
  - optional args to `report_energy`, never supplied
  - optional args to `offset_momentum`, never supplied
"""
...
def advance(dt:float, n:int)->Void:
    bodies=SYSTEM
    pairs=PAIRS
    for i in range(n):
        for (([x1, y1, z1], v1, m1),
...
def offset_momentum(ref:(List(float), List(float), float))->Void:
    bodies=SYSTEM
    px=0.0
    py=0.0
    pz=0.0
