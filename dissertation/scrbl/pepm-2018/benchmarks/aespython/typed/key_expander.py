...
@fields({"_key_length": int
        ,"_n": int
        ,"_expanded_key_length": Dict(int, int)})
class KeyExpander:
    """Perform AES Key Expansion"""

    _expanded_key_length = {128 : 176, 192 : 208, 256 : 240}

    def __init__(self:KeyExpander, key_length : Int)->Void:
    ...
    def _core(self:KeyExpander, key_array : List(Int), iteration : Int)->List(int):
        if len(key_array) != 4:
    ...
    def _xor_list(self:KeyExpander, list_1 : List(Int), list_2 : List(Int))->List(int):
        return [ i ^ j for i,j in zip(list_1, list_2)]

    def expand(self:KeyExpander, key_array : List(Int))->List(int):
        """
